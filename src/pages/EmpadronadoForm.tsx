...
const EmpadronadoForm: React.FC = () => {
    ...
    try {
      ...
      if (empadronado) {
        setFormData({
          ...
          telefonos:
            empadronado.telefonos && empadronado.telefonos.length > 0 ? empadronado.telefonos : [{ numero: "" }],
  ...
  const validateForm = async (): Promise<boolean> => {
    ...
    if (createUserAccount) {
      ...
      if (userAccountData.password.length < 6)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...
        ...(formData.miembrosFamilia?.filter(
          (miembro) => (miembro.nombre?.trim() && miembro.apellidos?.trim()) || miembro.nombre === "Menor de edad",
        ).length > 0 && {
          miembrosFamilia: formData.miembrosFamilia
            .filter(
              (miembro) => (miembro.nombre?.trim() && miembro.apellidos?.trim()) || miembro.nombre === "Menor de edad",
            )
            .map((miembro) => ({
        ...
        ...(formData.vehiculos?.filter((vehiculo) => vehiculo.placa?.trim()).length > 0 && {
          vehiculos: formData.vehiculos
            .filter((vehiculo) => vehiculo.placa?.trim())
            .map((vehiculo) => ({
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                type="date"
                value={
                  formData.fechaIngreso && !isNaN(formData.fechaIngreso)
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  <Switch id="isMinor" checked={isMinor} onCheckedChange={setIsMinor} />
                ...
                <div className="space-y-2">
                  {formData.miembrosFamilia.map((miembro, index) => (
                    <div key={index} className="flex items-center justify-between p-2 border rounded">
                      <span>
                        {miembro.nombre} {miembro.apellidos} - {miembro.parentezco} ({miembro.cumpleanos})
                      </span>
            ...
            {formData.tipoRegistro === "residente" && (
              <div>
                ...
                {formData.vehiculos?.map((vehiculo, index) => (
                  <div key={index} className="flex gap-2 mb-2">
                    <Input value={vehiculo.placa} readOnly placeholder="Placa" />
                    <Select value={vehiculo.tipo} disabled>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: "construida" | "construccion" | "terreno") =>
                      setFormData((prev) => ({ ...prev, estadoVivienda: value }))
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
            ...
            {hasExistingAccount ? (
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button variant="destructive" size="sm" disabled={unlinkingAccount}>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Esta acción desvinculará la cuenta <strong>{existingAccountInfo.emailAcceso}</strong> de
                            este empadronado. El usuario ya no podrá acceder al sistema con esta cuenta hasta que se
                            vincule nuevamente.
                            <br />
                            <br />
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={handleUnlinkAccount}>Desvincular</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  </div>
                ...
                <p className="text-sm text-muted-foreground">
                  Esta cuenta está vinculada y el asociado puede usarla para acceder al sistema. Puedes desvincularla si
                  necesitas crear una nueva cuenta o corregir la vinculación.
                </p>
              </div>
            ) : (
              ...
              <>
                <div className="flex flex-col gap-4">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="createUserAccount">Crear nueva cuenta de acceso</Label>
                  </div>
                  ...
                  <div className="space-y-4 pt-4 border-t">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Input
                            ...
                            onChange={(e) =>
                              setUserAccountData((prev) => ({ ...prev, confirmPassword: e.target.value }))
                            }
                        ...
                        {errors.userPasswordConfirm && (
                          <p className="text-sm text-destructive mt-1">{errors.userPasswordConfirm}</p>
                        )}
                      </div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                      <p className="text-sm text-blue-800">
                        ...
                        El asociado podrá usar estas credenciales para acceder al Portal del Asociado y gestionar sus
                        visitas, pagos y demás servicios.
                      </p>
                    </div>
                  </div>
                )}
        ...
        empadronado={
          id
            ? ({
                ...
                numeroPadron: formData.numeroPadron,
              } as any)
            : null
        }
