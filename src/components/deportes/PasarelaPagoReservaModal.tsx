...
interface PasarelaPagoReservaModalProps {
  open: boolean;
  ...
  onPagoConfirmado: (fechaPago: Date, archivoComprobante: File, nombreBanco: string, numeroOperacion: string) => void;
}
...
export const PasarelaPagoReservaModal = ({
  open,
  onOpenChange,
  montoTotal,
  detalleReserva,
  onPagoConfirmado,
}: PasarelaPagoReservaModalProps) => {
  ...
  const handleConfirmarPago = () => {
    ...
    setLoading(true);
    onPagoConfirmado(fechaPago, archivoComprobante, medioSeleccionado.nombre, numeroOperacion || "S/N");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Realizar Pago de Reserva</DialogTitle>
        </DialogHeader>
          ...
          <Card className="bg-primary/5 border-primary/20">
            <CardContent className="pt-6">
              <div className="space-y-2">
                <h3 className="font-semibold text-lg mb-3">Detalle de tu reserva</h3>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <p className="text-muted-foreground">Cancha:</p>
                  <p className="font-medium">{detalleReserva.cancha}</p>
                  
                  <p className="text-muted-foreground">Fecha:</p>
                  <p className="font-medium">{detalleReserva.fecha}</p>
                  
                  <p className="text-muted-foreground">Horario:</p>
                  <p className="font-medium">{detalleReserva.horaInicio} - {detalleReserva.horaFin}</p>
                  
                  <p className="text-muted-foreground">Duración:</p>
                  <p className="font-medium">{detalleReserva.duracion}h</p>
                  
                  <p className="text-muted-foreground">Con luz:</p>
                  <p className="font-medium">{detalleReserva.conLuz ? "Sí" : "No"}</p>
                </div>
                <div className="pt-4 border-t mt-4">
                  <div className="flex justify-between items-center">
                    <p className="text-lg font-semibold">Monto a pagar</p>
                    <p className="text-3xl font-bold text-primary">S/ {montoTotal.toFixed(2)}</p>
                  </div>
                </div>
              </div>
            </CardContent>
