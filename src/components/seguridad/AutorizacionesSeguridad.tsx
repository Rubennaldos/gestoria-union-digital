...
export const AutorizacionesSeguridad = () => {
  ...
  const { updateData } = useFirebaseWrite();
  const [autorizaciones, setAutorizaciones] = useState<AutorizacionPendiente[]>([]);
  
  // Obtener registros pendientes de autorización
  ...
  useEffect(() => {
    ...
    // Visitas pendientes
    if (visitas) {
      Object.entries(visitas).forEach(([id, visita]) => {
        if (visita.estado === 'pendiente') {
          pendientes.push({ id, tipo: 'visitante', data: visita, fechaCreacion: visita.fechaCreacion });
        }
      });
    }
    ...
    if (trabajadores) {
      Object.entries(trabajadores).forEach(([id, trabajador]) => {
        if (trabajador.estado === 'pendiente') {
          pendientes.push({ id, tipo: 'trabajador', data: trabajador, fechaCreacion: trabajador.fechaCreacion });
        }
      });
    }
    ...
    if (proveedores) {
      Object.entries(proveedores).forEach(([id, proveedor]) => {
        if (proveedor.estado === 'pendiente') {
          pendientes.push({ id, tipo: 'proveedor', data: proveedor, fechaCreacion: proveedor.fechaCreacion });
        }
      });
    }
  ...
  const manejarAutorizacion = async (id: string, tipo: string, autorizar: boolean) => {
    try {
      const path = `acceso/${tipo === 'visitante' ? 'visitas' : tipo === 'trabajador' ? 'trabajadores' : 'proveedores'}/${id}`;
      await updateData(path, {
        estado: autorizar ? 'autorizado' : 'denegado',
        fechaAutorizacion: Date.now(),
        autorizadoPor: 'seguridad'
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {autorizaciones.length} solicitudes esperando autorización
          </p>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription>
                          Solicitud del {new Date(auth.fechaCreacion).toLocaleString()}
                        </CardDescription>
                      </div>
                    </div>
                    <Badge className={getColorBadge(auth.tipo)}>
                      {auth.tipo.toUpperCase()}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-muted-foreground capitalize">
                          {auth.data.tipoAcceso}
                        </p>
                      </div>
                      {auth.data.placa && (
                        <div>
                          <span className="font-medium">Placa:</span>
                          <p className="text-muted-foreground">{auth.data.placa}</p>
                        </div>
                      )}
                      ...
                      {auth.tipo === 'visitante' && (
                        <>
                          ...
                          <div>
                            ...
                            <p className="text-muted-foreground">
                              {(auth.data as RegistroVisita).menores}
                            </p>
                          </div>
                        </>
                      )}
